...
def mat_presets_group_cloud(node_tree0, mat_out_0): 
        
    node_tree1 = bpy.data.node_groups.get('Cloud')
    if not node_tree1:
        node_tree1 = bpy.data.node_groups.new('Cloud', 'ShaderNodeTree')
        # INPUTS
        ...
        mix_shader_1.inputs[0].default_value = 0.25
        ...
        node_tree1.links.new(group_input_1.outputs[1], volume_scatter_1.inputs[1])
        node_tree1.links.new(group_input_1.outputs[1], volume_absorption_1.inputs[1])

    group_0 = node_tree0.nodes.new('ShaderNodeGroup')
    if hasattr(group_0, 'node_tree'):
        group_0.node_tree = bpy.data.node_groups.get('Cloud')
